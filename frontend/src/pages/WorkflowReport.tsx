...
const WorkflowReport = () => {
    ...
    return (
      <Layout className="report-layout">
        <div className="error-container">
          ...
          <Paragraph>{safeRenderError(error)}</Paragraph>
  ...
  const renderTrustScoreSection = () => {
    ...
    const dimensions: TrustScoreDimension[] = [
      ...
      },
      {
        ...
        feedback:
          reportData?.primaryDataRate && reportData.primaryDataRate >= 80
            ? "一手数据比例达标，数据可信度高"
            : "建议增加一手数据的使用比例",
      },
      {
        ...
        feedback:
          reportData?.verifiedDataRate && reportData.verifiedDataRate >= 80
            ? "数据验证率达标，结果可靠性高"
            : "建议提高数据的验证比例",
      },
    ];
      ...
      <section className="report-section">
        ...
        <Card className="trust-score-card">
          <Row gutter={[24, 24]} align="middle">
            <Col span={6}>
              <div className="trust-score-circle">
                <Progress
                  ...
                  width={120}
                  strokeColor={
                    overallDataQuality >= 80
                      ? "#52c41a"
                      : overallDataQuality >= 60
                        ? "#faad14"
                        : "#f5222d"
                  }
                />
            ...
            <Col span={18}>
              <Paragraph>
                根据数据完整性、一级数据比例和可验证性评估，模型的综合可信分为
                {Math.round(overallDataQuality)}分，
                {overallDataQuality >= 80
                  ? "达到了良好水平！"
                  : overallDataQuality >= 60
                    ? "基本达标，但仍有提升空间。"
  ...
  const renderMassBalanceSection = () => {
    ...
    const totalInputWeight = materialInputs.reduce(
    ...
    materialInputs.forEach((item) => {
      item.percentage =
        totalInputWeight > 0 ? (item.weight / totalInputWeight) * 100 : 0;
    });

    // 计算产品输出
    const productNodes = reportData.nodes.filter(
      (node: any) =>
        node.data.lifecycleStage === "生产" && node.data.nodeType === "product",
    );
    ...
    // 计算生产损耗
    const wasteNodes = reportData.nodes.filter(
      (node: any) =>
        node.data.lifecycleStage === "生产" && node.data.nodeType === "waste",
    );

    const totalWasteWeight = wasteNodes.reduce(
      (sum, node) => sum + (node.data.weight || 0),
      0,
    );
    ...
    const outputs = [
      {
        ...
        percentage:
          totalInputWeight > 0 ? (productWeight / totalInputWeight) * 100 : 0,
      },
      {
        key: "waste",
        output: "生产损耗",
        ...
        percentage:
          totalInputWeight > 0
            ? (totalWasteWeight / totalInputWeight) * 100
            : 0,
      },
    ];

    // 计算质量平衡率
    const massBalanceRatio =
      totalInputWeight > 0
        ? (productWeight + totalWasteWeight) / totalInputWeight
        : 0;

    const getMassBalanceStatus = (
      ratio: number,
    ): "success" | "warning" | "error" => {
        ...
        <Card className="mass-balance-card">
                  ...
                  summary={() => (
                    <Table.Summary.Row>
                      ...
                      <Table.Summary.Cell index={1}>
                        {totalInputWeight.toFixed(2)}
                      </Table.Summary.Cell>
                      <Table.Summary.Cell index={2}>100.0</Table.Summary.Cell>
                      <Table.Summary.Cell index={3}>
                        {materialInputs
                          .reduce((sum, item) => sum + item.carbonFootprint, 0)
                          .toFixed(2)}
                      </Table.Summary.Cell>
                    </Table.Summary.Row>
                  )}
                  ...
                  summary={() => (
                    <Table.Summary.Row>
                      ...
                      <Table.Summary.Cell index={2}>
                        {(
                          ((productWeight + totalWasteWeight) /
                            totalInputWeight) *
                          100
                        ).toFixed(1)}
                      </Table.Summary.Cell>
          ...
          <div style={{ marginTop: 24 }}>
            ...
            <Paragraph>
              <ul>
                <li>
                  <Text strong>物料投入：</Text> 共计{materialInputs.length}
                  种原材料， 总重量{totalInputWeight.toFixed(2)}kg
                </li>
                <li>
                  <Text strong>产品产出：</Text> 产品重量
                  {productWeight.toFixed(2)}kg， 产出率
                  {((productWeight / totalInputWeight) * 100).toFixed(1)}%
                </li>
                <li>
                  <Text strong>生产损耗：</Text> 损耗重量
                  {totalWasteWeight.toFixed(2)}kg， 损耗率
                  {((totalWasteWeight / totalInputWeight) * 100).toFixed(1)}%
                </li>
                <li>
                  <Text strong>质量平衡率：</Text>{" "}
                  {(massBalanceRatio * 100).toFixed(1)}%
                  {massBalanceStatus === "success"
                    ? "（达到平衡要求）"
                    : massBalanceStatus === "warning"
                      ? "（接近平衡）"
  ...
  const renderDataTraceabilitySection = () => {
    ...
    // 计算关键排放源覆盖率
    const nodes = reportData.nodes.filter(
      (node) => node.data && !node.id.includes("product-"),
    );
    const totalEmission = nodes.reduce(
      (sum, node: any) => sum + (node.data.carbonFootprint || 0),
      0,
    );
    ...
    // 计算一级数据占比
    const primaryDataNodes = nodes.filter(
      (node: any) =>
    ...
    const calculateDataQuality = (nodes: any[]) => {
      ...
      const withEvidence = nodes.filter(
        (node) =>
          node.data.dataSource && !node.data.dataSource.includes("未知"),
      ).length;

      const verifiable = nodes.filter(
        (node) =>
          node.data.dataSource?.includes("实测") ||
    ...
    const traceabilityData = sortedNodes
      ...
      .map((node: any) => ({
        ...
        dataType: node.data.dataSource?.includes("实测")
          ? "一级数据"
          : node.data.dataSource?.includes("数据库")
            ? "二级数据"
            : "三级数据",
        evidence: node.data.dataSource || "未知",
        status: node.data.dataSource?.includes("实测")
          ? "verified"
          : node.data.dataSource?.includes("数据库")
            ? "verified"
            : node.data.dataSource?.includes("待验证")
              ? "pending"
              : "unverified",
        completeness:
          node.data.weight &&
                  ...
                  <Row gutter={[16, 16]}>
                    ...
                    <Col span={12}>
                      <Statistic
                        ...
                        suffix="%"
                        valueStyle={{
                          color: primaryDataRate >= 70 ? "#52c41a" : "#faad14",
                        }}
                      />
                    </Col>
                  </Row>
                  <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
                    <Col span={8}>
                      <Progress
                        ...
                        format={() => "证明材料"}
                        strokeColor={
                          dataQuality.evidence >= 80 ? "#52c41a" : "#faad14"
                        }
                      />
                    </Col>
                    <Col span={8}>
                      <Progress
                        ...
                        format={() => "可核实性"}
                        strokeColor={
                          dataQuality.verifiable >= 80 ? "#52c41a" : "#faad14"
                        }
                      />
                    </Col>
                    <Col span={8}>
                      <Progress
                        ...
                        format={() => "完整性"}
                        strokeColor={
                          dataQuality.complete >= 80 ? "#52c41a" : "#faad14"
                        }
                      />
                    </Col>
                  </Row>
            ...
            <Row gutter={[24, 24]}>
              <Col span={8}>
                ...
                >
                  {primaryDataRate < 70 && (
                    <li>
                      <Text strong>提高一级数据比例</Text>
                      ：增加实测和直接测量数据的使用
                    </li>
                  )}
                  {dataQuality.evidence < 80 && (
                    <li>
                      <Text strong>完善证明材料</Text>
                      ：建立完整的数据来源文档体系
                    </li>
                  )}
                  {keyNodes.some(
                    (node: any) => !node.data.dataSource?.includes("实测"),
                  ) && (
                    <li>
                      <Text strong>关键排放源数据</Text>
                      ：优先提升主要排放源的数据质量
                    </li>
                  )}
                </Card>
              ...
              <Col span={8}>
                ...
                >
                  {dataQuality.verifiable < 80 && (
                    <li>
                      <Text strong>提高可核实性</Text>：建立数据验证流程
                    </li>
                  )}
                  {dataQuality.complete < 90 && (
                    <li>
                      <Text strong>提高数据完整性</Text>：补充缺失的关键参数
                    </li>
                  )}
                  <li>
                    <Text strong>标准化数据收集</Text>：统一数据收集格式和流程
                  </li>
                </Card>
              </Col>
              <Col span={8}>
          ...
          <div className="report-container">
            <div className="report-header-section">
              ...
              <div className="report-meta">
                ...
                <br />
                <Text type="secondary">
                  应用标准: ISO 14040, ISO 14044, ISO 14067
                </Text>
              </div>
            ...
            <section className="report-section">
              ...
              <Row gutter={[24, 24]} className="summary-cards">
                <Col span={8}>
                  <Card>
                    <Statistic
                      title="总碳足迹"
                      value={safeStatisticValue(
                        reportData?.totalCarbonFootprint,
                        0,
                      )}
                      precision={2}
                      suffix="kgCO₂e"
              ...
              <Row gutter={[24, 24]} style={{ marginTop: "24px" }}>
                <Col span={24}>{renderTrustScoreSection()}</Col>
              </Row>
            </section>
            ...
            <section className="report-section">
              ...
              <Card
                title="生命周期阶段碳排放分析"
                className="lifecycle-emissions-card"
              >
                <Row gutter={[24, 24]}>
                  <Col span={12}>
                    <ReactECharts
                      option={getLifecycleEmissionsChartOption(
                  ...
                  <Col span={12}>
                    <Table
                      dataSource={Object.keys(
                        ...
                        stage: stage,
                        carbonFootprint:
                          reportData?.stageEmissions[stage].carbonFootprint ||
                          0,
                        percentage:
                          reportData?.stageEmissions[stage].percentage || 0,
                        nodesCount:
                          reportData?.stageEmissions[stage].nodesCount || 0,
                      }))}
                      columns={[
                        {
                          title: "生命周期阶段",
              ...
              <Card title="碳排放热点分析" style={{ marginTop: 24 }}>
                <Row gutter={[24, 24]}>
                  <Col span={12}>
                    <ReactECharts
                      option={getHotspotParetoChartOption(
                  ...
                  <Col span={12}>
                      ...
                      reportData.hotspotNodes.map(
                        (node: any, index: number) => (
                          ...
                          >
                            ...
                            <Text strong>
                              {node.name} ({node.stage}阶段)
                            </Text>
                            <br />
                            <Text>
                              碳排放量: {node.carbonFootprint.toFixed(2)}{" "}
                              kgCO₂e，占总排放的 {node.percentage.toFixed(1)}%
                            </Text>
                          </div>
                        ),
                      )}
                    ...
                    <Paragraph style={{ marginTop: 16 }}>
                      ...
                      {reportData?.hotspotNodes
                        ? reportData.hotspotNodes
                            .reduce(
                              (sum: number, node: any) => sum + node.percentage,
                              0,
                            )
                            .toFixed(1)
                        : 0}
              ...
              <Card title="数据来源分析" style={{ marginTop: 24 }}>
                <Row gutter={[24, 24]}>
                  <Col span={12}>
                    <ReactECharts
                      option={getDataSourceDistributionChartOption(
                        reportData?.carbonFactorSources || {},
                      )}
                      style={{ height: 300 }}
                    />
                  </Col>
                  <Col span={12}>
                    <Table
                      dataSource={Object.entries(
                        ...
                        source,
                        count,
                        percentage: reportData?.nodes.length
                          ? ((count as number) / reportData.nodes.length) * 100
                          : 0,
                      }))}
                      columns={[
                        {
                    ...
                    <Paragraph>
                      {(() => {
                        ...
                        const totalCount = Object.values(sources).reduce(
                        ...
                        const percentage =
                          totalCount > 0
                            ? (highQualitySources / totalCount) * 100
                            : 0;

                        return `${percentage.toFixed(
                          1,
                        )}%的数据来自高质量来源，包括实测数据、数据库匹配等。`;
                      })()}
                      根据ISO 14064和GHG
                      Protocol标准，碳足迹计算的数据质量直接影响结果的可信度。
                    </Paragraph>
                  </Col>
                </Row>
              </Card>
                    ...
                    const dimensions: TrustScoreDimension[] = [
                      {
                        ...
                        status: calculateStatus(
                          reportData?.dataCompleteness ?? 0,
                          100,
                        ),
                        feedback:
                          reportData?.dataCompleteness &&
                          reportData.dataCompleteness >= 80
                            ? "数据完整性良好，已满足认证要求"
                            : "建议补充缺失的数据，提高完整性",
                      },
                      {
                        ...
                        maxScore: 100,
                        status: calculateStatus(
                          reportData?.primaryDataRate ?? 0,
                          100,
                        ),
                        feedback:
                          reportData?.primaryDataRate &&
                          reportData.primaryDataRate >= 80
                            ? "一手数据比例达标，数据可信度高"
                            : "建议增加一手数据的使用比例",
                      },
                      {
                        ...
                        maxScore: 100,
                        status: calculateStatus(
                          reportData?.verifiedDataRate ?? 0,
                          100,
                        ),
                        feedback:
                          reportData?.verifiedDataRate &&
                          reportData.verifiedDataRate >= 80
                            ? "数据验证率达标，结果可靠性高"
                            : "建议提高数据的验证比例",
                      },
                    ];
                          ...
                          <Col span={6}>
                            <div className="trust-score-circle">
                              <Progress
                                type="circle"
                                percent={
                                  reportData?.overallDataQuality
                                    ? Math.round(reportData.overallDataQuality)
                                    : 0
                                }
                                format={(percent?: number) => `${percent}分`}
                                width={120}
                                strokeColor={
                                  reportData?.overallDataQuality &&
                                  reportData.overallDataQuality >= 80
                                    ? "#52c41a"
                                    : reportData?.overallDataQuality &&
                                        reportData.overallDataQuality >= 60
                                      ? "#faad14"
                                      : "#f5222d"
                                }
                              />
                              <div className="trust-score-title">
                                总体数据质量评分
                              </div>
                            </div>
                          </Col>
                ...
                <TabPane tab="评分标准说明" key="rules">
                  <Card>
                    ...
                    <Paragraph>
                      ...
                      对于TO B产品需选择从摇篮到大门（包含阶段1+2），对于TO
                      C产品需选择从摇篮到坟墓（包含阶段1+2+3+4+5）。
                    </Paragraph>
                      ...
                      <Col span={16}>
                        <Table
                          ...
                          dataSource={
                            reportData?.nodes
                              ?.filter(
                                (node) => node.data.lifecycleStage === "原材料",
                              )
                              .map((node) => ({
                                key: node.id,
                                name:
                      ...
                      <Col span={16}>
                        <Table
                          ...
                          dataSource={
                            reportData?.nodes
                              ?.filter(
                                (node) => node.data.lifecycleStage === "生产",
                              )
                              .map((node) => ({
                                key: node.id,
                      ...
                      <Col span={16}>
                        <Table
                          ...
                          dataSource={
                            reportData?.nodes
                              ?.filter(
                                (node) => node.data.lifecycleStage === "分销",
                              )
                              .map((node) => ({
                                key: node.id,
                                name: node.data.productName || "未命名",
                      ...
                      <Col span={16}>
                        <Table
                          ...
                          dataSource={
                            reportData?.nodes
                              ?.filter(
                                (node) => node.data.lifecycleStage === "使用",
                              )
                              .map((node) => ({
                                key: node.id,
                                name: node.data.productName || "未命名",
                      ...
                      <Col span={16}>
                        <Table
                          ...
                          dataSource={
                            reportData?.nodes
                              ?.filter(
                                (node) => node.data.lifecycleStage === "处置",
                              )
                              .map((node) => ({
                                key: node.id,
                                name: node.data.productName || "未命名",
            ...
            <section className="report-section">
              ...
              <Row gutter={[24, 24]}>
                <Col span={12}>{renderPlaceholderChart("数据来源分布图")}</Col>
                <Col span={12}>{renderPlaceholderChart("不确定性分布图")}</Col>
              </Row>
                          ...
                          <div key={node.id} className="hotspot-item">
                            ...
                            <Text>
                              {node.stage === "原材料" &&
                                "建议使用回收材料或寻找低碳替代材料。"}
                              {node.stage === "生产" &&
                                "建议优化生产工艺，提高能源效率，使用可再生能源。"}
                              {node.stage === "分销" &&
                                "建议优化运输路线，选择低碳运输方式，改进包装设计。"}
                              {node.stage === "使用" &&
                                "建议提高产品使用阶段能效，延长使用寿命。"}
                              {node.stage === "处置" &&
                                "建议提高产品可回收性，设计易于拆解的结构。"}
                            </Text>
                          </div>
                        ),
                      )}
                ...
                <Col span={24}>
                  <Card title="减碳路径规划" className="improvement-card">
                    ...
                    <Paragraph>
                      ...
                      最大的减排机会存在于
                      {getMaxEmissionStage(reportData?.stageEmissions)}
                      阶段，应优先关注。
                    </Paragraph>
                  </Card>
                </Col>
            ...
            <section className="report-section">
              ...
              <Card className="standard-card">
                    ...
                    >
                      ...
                      <div className="compliance-item">
                        <Tag
                          color={
                            reportData?.dataCompleteness &&
                            reportData.dataCompleteness > 90
                              ? "green"
                              : "orange"
                          }
                        >
                    ...
                    >
                      ...
                      <div className="compliance-item">
                        <Tag
                          color={
                            reportData?.verifiedDataRate &&
                            reportData.verifiedDataRate > 65
                              ? "green"
                              : "orange"
                          }
                        >
                ...
                <div style={{ marginTop: 24 }}>
                  ...
                  <Paragraph>
                    本报告中的产品碳足迹评估整体符合ISO
                    14040、14044、14067和14064等国际标准的要求。
                  ...
                  <ul>
                    <li>
                      ...
                      {reportData?.dataCompleteness?.toFixed(1) || 0}%
                      {reportData?.dataCompleteness &&
                      reportData.dataCompleteness >= 90
                        ? "（优）"
                        : reportData?.dataCompleteness &&
                            reportData.dataCompleteness >= 75
                          ? "（良）"
                          : "（需改进）"}
                    </li>
                    <li>
                      <Text strong>一级数据比例：</Text>{" "}
                      {reportData?.primaryDataRate?.toFixed(1) || 0}%
                      {reportData?.primaryDataRate &&
                      reportData.primaryDataRate >= 70
                        ? "（优）"
                        : reportData?.primaryDataRate &&
                            reportData.primaryDataRate >= 50
                          ? "（良）"
                          : "（需改进）"}
                    </li>
                    <li>
                      <Text strong>数据验证率：</Text>{" "}
                      {reportData?.verifiedDataRate?.toFixed(1) || 0}%
                      {reportData?.verifiedDataRate &&
                      reportData.verifiedDataRate >= 65
                        ? "（优）"
                        : reportData?.verifiedDataRate &&
                            reportData.verifiedDataRate >= 45
                          ? "（良）"
                          : "（需改进）"}
                    </li>
                    <li>
                      <Text strong>总体数据质量评分：</Text>{" "}
                      {reportData?.overallDataQuality?.toFixed(1) || 0}%
                      {reportData?.overallDataQuality &&
                      reportData.overallDataQuality >= 75
                        ? "（优）"
                        : reportData?.overallDataQuality &&
                            reportData.overallDataQuality >= 60
                          ? "（良）"
            ...
            <section className="report-section">
              ...
              <Card>
                <Paragraph>
                  本报告基于ISO
                  14040/14044/14067标准框架，对产品进行了全生命周期碳足迹评估。
                  分析结果显示，产品总碳足迹为
                  {reportData?.totalCarbonFootprint?.toFixed(2) || 0} kgCO₂e，
                  碳强度为{reportData?.carbonIntensity?.toFixed(2) || 0}{" "}
                  kgCO₂e/kg。
                  {reportData?.stageEmissions && (
                    <>
                      主要碳排放热点集中在
                      {getMaxEmissionStage(reportData.stageEmissions)}阶段，
                      占总碳足迹的
                      {getMaxEmissionPercentage(reportData.stageEmissions)}%。
                    </>
                  )}
                </Paragraph>
                <Paragraph>
              ...
              <Card className="nodes-details-card">
                <Table
                  dataSource={reportData?.nodes
                    ?.filter(
                      (node: any) => node.data && !node.id.includes("product-"),
                    )
                    .map((node: any) => ({
                      key: node.id,
                      id: node.id,
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      filters: reportData?.carbonFactorSources
                        ? Object.keys(reportData.carbonFactorSources).map(
                  ...
                  summary={(pageData) => {
                    ...
                    const totalWeight = pageData.reduce(
                      (sum, node: any) => sum + node.weight,
                      0,
                    );
                    const totalCarbonFootprint = pageData.reduce(
                      (sum, node: any) => sum + node.carbonFootprint,
                      0,
                    );
                  ...
                  <Row gutter={[16, 16]}>
                    <Col span={8}>
                      <Statistic
                        title="总节点数"
                        value={
                          reportData?.nodes?.filter(
                            (node: any) => !node.id.includes("product-"),
                          ).length || 0
                        }
                      />
                    </Col>
                      ...
                      <Statistic
                        ...
                        value={
                          reportData?.nodes?.filter(
                            (node: any) =>
                              !node.id.includes("product-") &&
                              node.data.dataSource?.includes("需要人工介入"),
                          ).length || 0
                        }
