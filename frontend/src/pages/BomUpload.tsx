...
const BomUpload: React.FC = () => {
  ...
  // 解析上传的 BOM 内容
  const originalBomTable = uploadedBom
    ? csvToTableData(uploadedBom.content)
    : { columns: [], dataSource: [] };
  ...
  return (
    <Layout style={{ minHeight: "100vh", background: "#f0f2f5" }}>
      ...
      >
        ...
        <Text type="secondary">
          上传您的 BOM (物料清单)
          文件，系统将自动分析并标准化格式，便于碳足迹计算。 支持 CSV 和 Excel
          格式。
        </Text>
        ...
        <Card style={{ marginTop: "24px" }}>
          <Dragger {...uploadProps} style={{ marginBottom: "16px" }}>
            ...
            <p className="ant-upload-hint">支持 CSV 和 Excel 格式的 BOM 文件</p>
          </Dragger>
          ...
          <div style={{ marginTop: "16px", textAlign: "center" }}>
            ...
            <Button type="default" onClick={() => navigate("/boms")}>
              返回 BOM 列表
            </Button>
          </div>
        </Card>
            ...
            {uploadedBom.standardized_content && (
              <>
                ...
                <Button
                  type="link"
                  onClick={() =>
                    handlePreview(uploadedBom.standardized_content || "")
                  }
                  icon={<FileTextOutlined />}
                >
                  查看标准化内容
