...
import { Avatar, Badge, Button, Card, Col, Divider, Form, Input, Layout, List, Menu, message, Modal, Row, Select, Spin, Statistic, Switch, Tabs, Tag, Typography, notification, Tooltip } from 'antd';
...
const industryTemplates: IndustryTemplate[] = [
  {
    ...
    name: "电子产品标准流程",
    industry: "电子制造",
    description: "适合消费电子产品的碳足迹分析流程，包含元器件生产、组装和运输环节",
...
const Dashboard: React.FC = () => {
  ...
  const refreshData = async () => {
    try {
      ...
      const workflowsWithIndustry = workflowsRes.data.map((workflow: Workflow) => {
        const enrichedWorkflow = {
          ...workflow,
          industry_type: industries[Math.floor(Math.random() * industries.length)]
        };
  ...
  const filteredWorkflows = workflows.filter(workflow => {
    ...
    const matchesIndustry = industryFilter === 'all' || workflow.industry_type === industryFilter;
  ...
  const renderDashboard = () => {
    // 计算平均碳足迹
    const avgCarbonFootprint = products.length > 0 
      ? products.reduce((sum, product) => sum + product.carbon_footprint, 0) / products.length
      : 0;
    ...
    return (
      <>
        ...
        <Card className="company-profile" style={{ marginBottom: '24px', backgroundColor: '#f8f9fa' }}>
          <Row gutter={16} align="middle">
            <Col span={4}>
              <Avatar size={80} style={{ backgroundColor: '#87d068' }}>ESG</Avatar>
            </Col>
            <Col span={20}>
        ...
        <Card title="碳足迹热点分析">
          <Tabs defaultActiveKey="1">
            <TabPane tab="生命周期阶段" key="1">
              <div style={{ height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f5' }}>
                <Text type="secondary">此处将显示各生命周期阶段碳足迹占比图表</Text>
              </div>
            </TabPane>
            ...
            <TabPane tab="改进建议" key="3">
              <List
                ...
                dataSource={[
                  { title: '优化供应链物流', description: '通过整合运输路线，可减少碳排放约12%', impact: 'high' },
                  { title: '更换低碳原材料', description: '使用回收塑料替代原生塑料，可减少碳排放约18%', impact: 'high' },
                  { title: '提升能源效率', description: '工厂设备能效提升可减少生产阶段碳排放约15%', impact: 'medium' },
                  { title: '包装减量化', description: '优化产品包装设计，减少材料使用可降低2-5%碳足迹', impact: 'low' }
                ]}
                renderItem={item => (
                  <List.Item>
                    <List.Item.Meta
                      avatar={<BulbOutlined style={{ fontSize: '24px', color: '#1890ff' }} />}
                      title={<span>{item.title} <Badge color={item.impact === 'high' ? 'red' : item.impact === 'medium' ? 'orange' : 'green'} text={item.impact === 'high' ? '高影响' : item.impact === 'medium' ? '中等影响' : '低影响'} /></span>}
  ...
  const renderWorkbench = () => {
    return (
      <>
        ...
        <Card title="我的产品碳足迹工作流" extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/workflow/new')}>创建新工作流</Button>}>
          ...
          <List
            ...
            renderItem={item => (
              <List.Item
                key={item.id}
                actions={[
                  <Button type="link" onClick={() => navigate(`/workflow/${item.id}`)}>
                    编辑
                  </Button>,
              ...
              >
                <List.Item.Meta
                  ...
                  title={<>
                    {item.name} 
                    {item.industry_type && <Tag color="blue" style={{ marginLeft: '8px' }}>{item.industry_type}</Tag>}
                  </>}
                  description={
                    <>
                      <Text type="secondary">{item.description}</Text>
                      <br />
                      <Text type="secondary">碳足迹: {item.total_carbon_footprint.toFixed(2)} kg CO₂e</Text>
                      <Text type="secondary" style={{ marginLeft: '16px' }}>创建时间: {new Date(item.created_at).toLocaleDateString()}</Text>
                    </>
                  }
                />
              </List.Item>
                ...
                >
                  ...
                  <div style={{ marginTop: '16px' }}>
                    <Button type="primary" block onClick={() => {
                      message.success(`已选择 ${item.name} 模板，正在创建工作流...`);
        ...
        <Tabs defaultActiveKey="1">
              ...
              <Row gutter={16}>
                <Col span={6}>
                  <div style={{ textAlign: 'center' }}>
                    <Avatar size={100} style={{ backgroundColor: '#1890ff' }}>用户</Avatar>
                    <div style={{ marginTop: '16px' }}>
                ...
                <Col span={18}>
                  <Form layout="vertical" initialValues={{ email: 'user@example.com', username: '张三', company: '绿色科技有限公司', role: '可持续发展经理' }}>
                    <Row gutter={16}>
                      <Col span={12}>
          ...
          <TabPane tab="通知设置" key="3">
            <Card>
              <List
                itemLayout="horizontal"
                dataSource={[
                  { title: '产品碳足迹提醒', description: '当产品碳足迹超过行业平均值时发出提醒', checked: true },
                  { title: '报告生成通知', description: '当新的碳足迹报告生成完成时通知', checked: true },
                  { title: '系统更新', description: '接收有关系统功能更新的通知', checked: false },
                  { title: '数据导出完成', description: '当数据导出任务完成时发送通知', checked: true }
                ]}
                renderItem={item => (
                  <List.Item
                    actions={[
              ...
              <Row gutter={16}>
                <Col span={12}>
                  <Card title="数据导出" bordered={false}>
                    <Text>导出企业碳足迹数据和分析报告，用于外部报告或进一步分析。</Text>
                    <div style={{ marginTop: '16px' }}>
                      <Select defaultValue="all" style={{ width: 200, marginRight: '16px' }}>
                        <Select.Option value="all">全部数据</Select.Option>
                        <Select.Option value="products">仅产品数据</Select.Option>
                        <Select.Option value="workflows">仅工作流数据</Select.Option>
                        <Select.Option value="reports">仅报告数据</Select.Option>
                      </Select>
                ...
                <Col span={12}>
                  <Card title="数据备份" bordered={false}>
                    <Text>创建企业数据备份，确保碳足迹计算和分析数据安全。</Text>
                    <div style={{ marginTop: '16px' }}>
          ...
          <TabPane tab="关于系统" key="6">
            <Card>
              ...
              <Paragraph>
                ESG AI是一个帮助企业计算、分析和管理产品碳足迹的智能平台。利用人工智能技术，
              ...
              <List
                itemLayout="horizontal"
                dataSource={[
                  { title: '智能碳足迹计算', description: '基于AI的碳足迹计算引擎，支持多种产品和行业' },
                  { title: 'BOM智能解析', description: '自动解析物料清单，识别碳足迹热点' },
                  { title: '行业基准对比', description: '与行业基准进行对比，找出改进空间' },
                  { title: '减排策略建议', description: 'AI生成的减排策略建议，帮助企业实现可持续发展目标' }
                ]}
                renderItem={item => (
                  <List.Item>
                    <List.Item.Meta
