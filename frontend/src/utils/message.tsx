...
const ensureStringContent = (content: any): string => {
  ...
  // 处理常见的原始类型
  if (typeof content === 'number' || typeof content === 'boolean' || typeof content === 'symbol') {
  ...
  if (typeof content === 'object') {
    ...
    if (content.detail) {
      ...
      if (Array.isArray(content.detail)) {
        try {
          return content.detail.map((err: any) => {
            if (err && err.msg) {
              let field = err.loc && Array.isArray(err.loc) ? err.loc.slice(1).join('.') : '';
    ...
    // 其他情况尝试JSON序列化
    try {
      return typeof content.toString === 'function' && content.toString !== Object.prototype.toString
        ? content.toString()
...
// 封装消息配置，确保内容是字符串
const safeConfig = (content: any, options?: Omit<ArgsProps, 'content'>): ArgsProps => {
  // 如果传入的是配置对象
  if (typeof content === 'object' && !React.isValidElement(content) && !(content instanceof Error)) {
