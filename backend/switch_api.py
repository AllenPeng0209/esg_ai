...
# 目标文件
TARGET_FILE = os.path.join(os.path.dirname(__file__), "app", "services", "ai_service.py")
...
def restart_backend_service():
    ...
    try:
        ...
        result = subprocess.run(["docker-compose", "restart", "backend"], 
...
def main():
    ...
    parser.add_argument("--service", choices=["openai", "deepseek", "mock"], 
                        help="切换API服务 (openai, deepseek, mock)")
    
    parser.add_argument("--mock", choices=["on", "off"], 
                        help="启用或禁用始终使用模拟响应")
    
    parser.add_argument("--fallback", choices=["on", "off"], 
                        help="启用或禁用自动降级到模拟模式")
