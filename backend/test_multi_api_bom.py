...
DEEPSEEK_API_KEY = settings.DEEPSEEK_API_KEY if hasattr(settings, 'DEEPSEEK_API_KEY') else settings.OPENAI_API_KEY
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    self.logger.error(f"API调用失败: {response.status_code}, 内容: {response.text}")
