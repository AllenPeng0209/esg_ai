...
def get_vendor_tasks(
    ...
    response = (
        supabase.table("vendor_tasks").select("*").range(skip, skip + limit).execute()
    )
...
def get_pending_tasks(
    ...
    supabase = get_supabase_client()
    response = (
        supabase.table("vendor_tasks").select("*").eq("status", "pending").execute()
    )
    return response.data
...
def create_vendor_task(
    ...
    if workflow.data["user_id"] != str(current_user.id):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to create tasks in this workflow",
        )
...
def update_vendor_task(
    ...
    workflow = (
        supabase.table("workflows")
        .select("*")
        .eq("id", str(task.data["workflow_id"]))
        .single()
        .execute()
    )
