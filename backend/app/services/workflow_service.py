...
def create_workflow(workflow: WorkflowCreate, user_id: UUID) -> dict:
    ...
    if workflow.nodes:
        for node in workflow.nodes:
            ...
            if isinstance(node, dict):
                ...
                node_type = getattr(node, "type", None) or getattr(
                    node, "node_type", None
                )
        ...
        if nodes_data:
            nodes_response = (
                supabase.table("workflow_nodes").insert(nodes_data).execute()
            )
    ...
    if workflow.edges:
        ...
        if edges_data:
            edges_response = (
                supabase.table("workflow_edges").insert(edges_data).execute()
            )
...
def update_workflow(
    workflow_id: UUID, workflow_data: WorkflowUpdate, user_id: UUID
) -> Optional[dict]:
    """Update workflow"""
    supabase = get_supabase_client()
    ...
    update_data = {
        k: v
        for k, v in workflow_data.dict(exclude_unset=True).items()
    ...
    if workflow_data.nodes is not None:
        ...
        supabase.table("workflow_nodes").delete().eq(
    ...
    if workflow_data.edges is not None:
        ...
        supabase.table("workflow_edges").delete().eq(
            "workflow_id", str(workflow_id)
        ).execute()
...
def update_workflow_node(
    node_id: UUID, node_data: WorkflowNodeUpdate
) -> Optional[dict]:
...
def update_workflow_edge(
    edge_id: UUID, edge_data: WorkflowEdgeUpdate
) -> Optional[dict]:
