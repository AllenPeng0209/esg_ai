...
def get_bom_file_by_id(
    db: Session, bom_id: int, user_id: Optional[int] = None
) -> Optional[BOMFile]:
...
def get_user_bom_files(
    db: Session, user_id: int, skip: int = 0, limit: int = 100
) -> List[BOMFile]:
    """获取用户的所有BOM文件"""
    return (
        db.query(BOMFile)
        .filter(BOMFile.user_id == user_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
async def standardize_bom_file(
    db: Session, bom_id: int, user_id: int
) -> Optional[BOMFile]:
    """标准化BOM文件内容"""
