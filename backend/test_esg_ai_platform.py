...
class ESGAITestRunner:
    ...
    async def test_login_fail(self):
        ...
        try:
            ...
            test.add_step("准备无效登录数据", True, f"用户名: {wrong_username}, 密码: {wrong_password}")
    ...
    async def test_openai_proxy_api(self):
        ...
        test = TestResult(
            name="OpenAI代理API测试",
            description="验证OpenAI代理API的模拟响应功能",
            category="API"
        )
        test.start()
        ...
        try:
            ...
            test.add_step(
                "准备请求数据", 
                True,
                f"消息: {request_data['messages'][0]['content']}"
            )
            
            # 步骤2：发送POST请求到OpenAI代理
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{API_BASE_URL}/ai/test-openai-proxy",
                    json=request_data
                )
                ...
                try:
                    ...
                    test.add_step(
                        "验证响应包含消息内容", 
                        has_message,
                        f"消息: {response_data['choices'][0]['message']['content'][:50]}..." if has_message else "未找到消息内容"
                    )
                except Exception as parse_error:
                    test.add_step(
                        "解析响应JSON", 
...
3,有机白砂糖,7445.04,0,碳中和产品，上游过程已实现碳中和"""
            ...
            test.add_step(
                "准备BOM数据", 
                True,
                f"BOM数据包含 {bom_data.count(chr(10))+1} 行"
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    test.add_step(
                        "验证响应包含消息内容", 
                        has_message,
                        f"消息前50字符: {response_data['choices'][0]['message']['content'][:50]}..." if has_message else "未找到消息内容"
                    )
    ...
    async def test_mock_response(self):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                response_data1 = response1.json() if response1.status_code == 200 else {}
            ...
            async with httpx.AsyncClient() as client:
                ...
                response_data2 = response2.json() if response2.status_code == 200 else {}
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                test.add_step(
                    "验证页面标题", 
                    has_title,
                    "找到页面标题" if has_title else "未找到页面标题"
                )
